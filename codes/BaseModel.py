...
class BaseModel(BertPreTrainedModel):
    ...
    def setting_preparation(self, input_ids=None, attention_mask=None,
        ...
        if attention_mask.dim() == 3:
            ...
            if self.config.is_decoder:
                ...
                causal_mask = seq_ids[None, None, :].repeat(batch_size, seq_length,
                                                            1) <= seq_ids[None, :, None]
        ...
        if head_mask is not None:
            if head_mask.dim() == 1:
                head_mask = head_mask.unsqueeze(0).unsqueeze(0).unsqueeze(-1).unsqueeze(
                    -1)
                head_mask = head_mask.expand(self.config.num_hidden_layers, -1, -1, -1,
                                             -1)
    ...
    def forward(self, init_pos_ids, hop_dis_ids, time_dis_ids, head_mask=None):
        ...
        encoder_outputs = self.encoder(embedding_output,
                                       head_mask=head_mask)  # 这里的输出是tuple，因为在某些设定下要输出别的信息（中间分析用）
